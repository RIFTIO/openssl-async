...
used to perform cryptographic operations. See L<SSL_get_error(3)>. If you use
this mode then you should first have initialised async operations on each thread
that it is used using L<ASYNC_init_pool(3)>, and deinitialise before the thread
closes using L<ASYNC_free_pool(3)>. Note that when using this mode any retries
must occur on the same thread as the initial call.
