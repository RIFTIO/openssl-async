...
ASYNC_set_wait_fd, ASYNC_clear_wait_fd, ASYNC_get_current_job,
ASYNC_block_pause, ASYNC_unblock_pause - asynchronous job management functions
...
=head1 SYNOPSIS
 ...
 void ASYNC_clear_wait_fd(ASYNC_JOB *job);
...
ASYNC_JOBs can have a "wait" file descriptor associated with them. Calling
ASYNC_get_wait_fd() and passing in a pointer to an ASYNC_JOB in the B<job>
parameter will return the wait file descriptor associated with that job or
OSSL_BAD_ASYNC_FD if there is no associated file descriptor. This
...
application will have to priodically "poll" the job by attempting to restart it
to see if it is ready to continue.

Applications can provide a file descriptor to use for waiting on by calling
ASYNC_set_wait_fd(). Typically this would be done by an engine immediately prior
to calling ASYNC_pause_job() and not by end user code. An existing association
with a file descriptor can be cleared using ASYNC_clear_wait_fd().

An example of typical usage might be an async capable engine. User code would
initiate cryptographic operations. The engine would initiate those operations
asynchronously and then call ASYNC_set_wait_fd() followed by ASYNC_pause_job()
to return control to the user code. The user code can then perform other tasks
or wait for the job to be ready by calling "select" or other similar function
on the wait file descriptor. The engine can signal to the user code that the job
should be resumed by making the wait file descriptor "readable". Once resumed
...
The following example demonstrates how to use most of the core async APIs:
 ...
 int jobfunc(void *arg)
 {
     ...
     ASYNC_set_wait_fd(currjob, pipefds[0]);
