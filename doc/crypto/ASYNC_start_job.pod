...
The ASYNC_JOB pool also has a "wait" file descriptor associated with it, to be
used in the case where the error ASYNC_NO_JOBS is returned from
ASYNC_start_job(). The file descriptor can be obtained by calling
ASYNC_get_pool_wait_fd(). This will be shown as ready for "read" to indicate
that jobs are available in the pool.
