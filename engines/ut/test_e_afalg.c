...
static void evp_cipher_ctx_init(EVP_CIPHER_CTX *ctx,
                                int clen, int klen, int nid, int op)
{
    EVP_CIPHER *c;
...
static void test_afalg_cipher_init(void)
{
    ...
    assert(ret == 0 && "Cipher Init Failed for NULL paramter for ctx");
    PRINT_INFO("%s: Passed NULL Parameter Test for ctx\n", s);

    ret = afalg_cipher_init(ctx, NULL, iv, 1);
    assert(ret == 0 && "Cipher Init Failed for NULL paramter for key");
    ...
    assert(cmp == 0 && "Cipher Init Failed for NULL paramter for iv");
...
static int test_afalg_do_cipher()
{
    ...
    ret = afalg_do_cipher(ctx, buf, rbuf, BUFSZ);
    assert(ret == 0
           && "test_afalg_do_cipher: Failed uninitialised cipher_data test");
    t++;
    PRINT_INFO
        ("test_afalg_do_cipher: Passed uninitialised cipher_data test\n");
    ...
    assert(memcmp(buf, rbuf, 16) != 0
           && "test_afalg_do_cipher: Failed Plain txt = Enc txt");
    t++;
    ...
    PRINT_ARRAY(dbuf, 16);
    PRINT_DBG("\n");
    assert(memcmp(buf, dbuf, 16) == 0
           && "test_afalg_do_cipher: Failed Dec(Enc(P)) != P");
    t++;
    ...
    t++;
    PRINT_INFO
        ("test_afalg_do_cipher: Passed single big block Encryption test\n");
    ...
    t++;
    PRINT_INFO
        ("test_afalg_do_cipher: Passed single big block Encryption Decryption Test\n");

    /*
     * Encrypt a big block in one go and Decrypt in chunks
     */
    memset(buf, 'B', BUFSZ);
    memset(rbuf, 'B', BUFSZ);
    evp_cipher_ctx_init(ctx, AES_IV_LEN, AES_KEY_SIZE_128, NID_aes_128_cbc,
    ...
    for (int c = 0; c < BUFSZ / ALIGN_B; c++) {
        ret =
            afalg_do_cipher(ctx, dbuf + (c * ALIGN_B), rbuf + (c * ALIGN_B),
                            ALIGN_B);
        if (ret < 1)
            assert
                ("test_afalg_do_cipher: Failed to decrypt single big block");
